...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("vector_add", &vector_add, "Element-wise vector addition (CUDA)", py::arg("a"),
        py::arg("b"));
  m.def("vector_add_raw", &vector_add_raw,
        "Element-wise vector addition (raw CUDA, no PyTorch alloc)", py::arg("a"), py::arg("b"));
  ...
  m.def("saxpy", &saxpy, "Compute z = a * x + y (CUDA)", py::arg("x"), py::arg("y"), py::arg("a"));
  m.def("matrix_add", &matrix_add, "Element-wise matrix addition (CUDA)", py::arg("a"),
        py::arg("b"));
  m.def("rgb_to_grayscale", &rgb_to_grayscale, "RGB to grayscale conversion (CUDA)",
        py::arg("inp"));
  m.def("transpose_naive", &transpose_naive, "Naive matrix transpose (CUDA)", py::arg("inp"));
